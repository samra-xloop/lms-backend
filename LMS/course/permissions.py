...
class CreatePermission(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if request.user.role.role == 'instructor':
            instructor = Author.objects.get(id=request.data.get('instructor'))  # Adjust how you retrieve the author as per your serializer
...
# class ListAuthorPermission(permissions.BasePermission):
#     def has_permission(self, request, view):
#         # Check if the user has permission to create a module
#         if request.user.role.role == 'admin' or (request.user.role.role == 'instructor' and Author.created_by  == request.user):   
#             return True
        
#         if Author.editor == request.user:
#             return True

#         # if request.user in Author.editor():
#         #     return True
...
#         # if request.user.role.role in ['admin', 'instructor']:
#         #     return True
#         if request.user.role.role =='admin' or (request.user.role.role == 'instructor' and obj.instructor.created_by == request.user):
#             return True
#         # Check if the user is the creator of the module
#         # if obj.created_by == request.user:
#         #     return True
#         # Check if the user is in the list of editors
#         if request.user in obj.instructor.editor.all():
#             return True
#         return False
